...
bool GetCloudNoise(vec3 tracePos, inout vec3 tracePosM, int cloudAltitude) {
    ...
    vec2 coord = GetRoundedCloudCoord(tracePosM.xz, 0.0);
    ...
    float threshold = max(abs(cloudAltitude - tracePos.y) / cloudStretch, 0.001);
    threshold = step(1.0, pow2(threshold));
...
vec4 GetVolumetricClouds(int cloudAltitude, float distanceThreshold, inout float cloudLinearDepth, float skyFade, float skyMult0, vec3 cameraPos, vec3 nPlayerPos, float lViewPosM, float VdotS, float VdotU, float dither) {
    ...
    for (int i = 0; i < sampleCount; i++) {
        ...
        if (GetCloudNoise(tracePos, tracePosM, cloudAltitude)) {
            ...
            cloudShading = mix(cloudShading, VdotSM1, 0.5);
                ...
                #ifdef DEFERRED1
                    light -= texture2D(colortex3, GetRoundedCloudCoord(cLightPos.xz, 0.001)).b * cloudShadingM;
                #else
                    light -= texture2D(gaux4, GetRoundedCloudCoord(cLightPos.xz, 0.001)).b * cloudShadingM;
                #endif
                cLightPos += gradientNoise * cLightPosAdd;
                #ifdef DEFERRED1
                    light -= texture2D(colortex3, GetRoundedCloudCoord(cLightPos.xz, 0.05)).b * cloudShadingM;
                #else
                    light -= texture2D(gaux4, GetRoundedCloudCoord(cLightPos.xz, 0.05)).b * cloudShadingM;
                #endif
...
//            float verticalFade = abs(cloudAltitude - tracePos.y) / cloudStretch;
//            verticalFade = max(1.0 - smoothstep(0.1, 1.0, pow2(verticalFade)), 0.1);
